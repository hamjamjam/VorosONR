...
for k = 1:6
    ...
    YLIM = [min(YLIMS{k,:})-50 max(YLIMS{k,:})+20];
    %filenames = {'Log13.txt', 'Log16.txt', 'Log18.txt'}; %-60,-20
    herepath = pwd;
    ...
    for j = 1:length(filenames)
        ...
        % Plot the profile, inferred velocity, and light transitions
        ...
        % PLOTS!
        hold on;
        fill([x,fliplr(x)]-profStartTime,2*180/pi * [y,bottom*ones(size(y))], [0.8,0.8,0.8],'LineStyle','none') % fills background in with light on/off data
        if plotAngVels
        ...
        %plot(trackpadTimes-profStartTime, -180/pi*allInferredValues(:,2), 'color', [0 0.5 0]);    % plots trackpad clicks
        ...
        if whichProf >0
            DATA = profileWriterFxn(T(whichProf,:), pad, transition, 0, 1);
        end
        plot((1:length(DATA))/90,DATA(:,1),'k'); % plots the velocity profile in black

        % plot the observer expectation
        ...
        xlim([0 105]); ylim(YLIM);
        ylabel({trialNoStr;'Angular Velocity [deg/s]'});
        xlabel('Time [sec]');
    ...
    if plotAngVels && plotObserver
        lgd0 = legend('Lights Off', 'Angular Velocity','Inferred Perceived Angular Velocity', 'Inferred No Motion', 'Inferred Motion Start', 'Commanded Profile',  'Observer Results',  'Location', 'SouthOutside', 'Orientation', 'Horizontal');
    elseif plotAngVels && ~plotObserver
        lgd0 = legend('Lights Off', 'Angular Velocity','Inferred Perceived Angular Velocity', 'Inferred No Motion', 'Inferred Motion Start','Commanded Profile',   'Location', 'SouthOutside', 'Orientation', 'Horizontal');
    elseif ~plotAngVels && plotObserver
        lgd0 = legend('Lights Off', 'Inferred Perceived Angular Velocity', 'Inferred No Motion', 'Inferred Motion Start','Commanded Profile', 'Observer Results',  'Location', 'SouthOutside', 'Orientation', 'Horizontal');
    elseif ~plotAngVels && ~plotObserver
        lgd0 = legend('Lights Off', 'Inferred Perceived Angular Velocity', 'Inferred No Motion', 'Inferred Motion Start', 'Commanded Profile',  'Location', 'SouthOutside', 'Orientation', 'Horizontal');
    end 
